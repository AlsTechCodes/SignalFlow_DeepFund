...
def calculate_hurst_exponent(price_series: pd.Series, max_lag: int = 20) -> float:
    ...
    Returns:
        float: Hurst exponent or None if calculation is not possible
    """
    # Input validation
    if not isinstance(price_series, (pd.Series, np.ndarray)):
        print("Warning: Input must be a pandas Series or numpy array")
        return 0.5

    # Convert to numpy array and remove NaN values
    prices = np.array(price_series).astype(float)
    prices = prices[~np.isnan(prices)]

    # Check if we have enough data points
    if len(prices) < max_lag + 1:
        print(f"Warning: Not enough data points ({len(prices)}) for max_lag={max_lag}")
        return 0.5
    ...
    try:
        ...
        # Remove infinite values that might occur from zero prices
        returns = returns[np.isfinite(returns)]
        
        if len(returns) < max_lag:
            print("Warning: Not enough valid returns after cleaning")
            return 0.5

        lags = range(2, min(max_lag + 1, len(returns)))
        
        # Calculate tau values with numerical stability
        tau = []
        for lag in lags:
            # Calculate price differences
            diffs = returns[lag:] - returns[:-lag]
            # Use nanstd to handle any remaining NaN values
            std_diff = np.nanstd(diffs)
            
            if std_diff > 0:  # Only include valid standard deviations
                tau.append(std_diff)
            
        if len(tau) < 2:
            print("Warning: Not enough valid tau values for regression")
            return 0.5
        ...
        # Use robust linear regression to reduce impact of outliers
        ...
        if not (0 <= hurst <= 1):
            print(f"Warning: Calculated Hurst exponent ({hurst}) outside valid range [0,1]")
            return min(max(hurst, 0), 1)
