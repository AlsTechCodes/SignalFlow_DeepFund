...
class Backtester:
    ...
    def parse_agent_response(self, agent_output):
        """Parse JSON output from the agent with improved error handling."""
        import json
        from typing import Dict, Any

        default_response = {"action": "hold", "quantity": 0, "reason": "Failed to parse agent response"}

        if not agent_output:
            print("Warning: Empty agent output received")
        ...
        try:
            if isinstance(agent_output, dict):
                ...
                print(f"Warning: Unexpected agent output type: {type(agent_output)}")
                return default_response
            ...
            if not all(field in decision for field in required_fields):
                ...
                print(f"Warning: Missing required fields in agent response: {missing}")
                return default_response

            # Validate action type
            valid_actions = ["buy", "sell", "hold"]
            if decision["action"].lower() not in valid_actions:
                print(f"Warning: Invalid action '{decision['action']}' in agent response")
                return default_response

            # Validate quantity is numeric
            try:
                decision["quantity"] = float(decision["quantity"])
            except (ValueError, TypeError):
                print(f"Warning: Invalid quantity value in agent response: {decision.get('quantity')}")
                return default_response
            ...
            print(f"Unexpected error parsing agent response: {str(e)}\nResponse: {agent_output}")
            return default_response
    ...
    def _update_performance_metrics(self, performance_metrics):
        """Helper method to update performance metrics using daily returns with improved error handling."""
        try:
            values_df = pd.DataFrame(self.portfolio_values).set_index("Date")
            
            # Ensure we have enough data points
            if len(values_df) < 2:
                print("Warning: Not enough data points to calculate performance metrics")
                return
            ...
            if len(clean_returns) < 2:
                print("Warning: Not enough valid returns to calculate performance metrics")
                return

            # Use a more realistic risk-free rate that can be updated
            daily_risk_free_rate = 0.0434 / 252  # Assuming 252 trading days
            excess_returns = clean_returns - daily_risk_free_rate
            
            # Calculate mean and standard deviation with numerical stability
            mean_excess_return = excess_returns.mean()
            std_excess_return = excess_returns.std(ddof=1)  # Use sample standard deviation
            
            # Sharpe Ratio calculation with validation
            if std_excess_return > 1e-8:  # Avoid division by very small numbers
                sharpe = np.sqrt(252) * (mean_excess_return / std_excess_return)
                # Bound Sharpe ratio to reasonable values
                performance_metrics["sharpe_ratio"] = min(max(sharpe, -100), 100)
            else:
                performance_metrics["sharpe_ratio"] = 0.0 if abs(mean_excess_return) < 1e-8 else np.inf * np.sign(mean_excess_return)
            ...
            if len(negative_returns) > 0:
                ...
                if downside_std > 1e-8:
                    sortino = np.sqrt(252) * (mean_excess_return / downside_std)
                    # Bound Sortino ratio to reasonable values
                    performance_metrics["sortino_ratio"] = min(max(sortino, -100), 100)
                else:
                    performance_metrics["sortino_ratio"] = 0.0 if abs(mean_excess_return) < 1e-8 else np.inf * np.sign(mean_excess_return)
            else:
                performance_metrics["sortino_ratio"] = np.inf if mean_excess_return > 0 else 0.0
            
            # Maximum drawdown calculation
            rolling_max = values_df["Portfolio Value"].expanding().max()
            drawdowns = (values_df["Portfolio Value"] - rolling_max) / rolling_max
            max_drawdown = drawdowns.min()
            
            # Ensure max drawdown is within valid range
            if np.isfinite(max_drawdown):
                performance_metrics["max_drawdown"] = max(max_drawdown * 100, -100)
            else:
                performance_metrics["max_drawdown"] = 0.0
                
            # Add additional metrics for portfolio analysis
            if "positions" in self.portfolio:
                total_long = sum(pos["long"] for pos in self.portfolio["positions"].values())
                total_short = sum(pos["short"] for pos in self.portfolio["positions"].values())
                
                # Long/Short ratio
                if total_short > 0:
                    performance_metrics["long_short_ratio"] = total_long / total_short
                else:
                    performance_metrics["long_short_ratio"] = np.inf if total_long > 0 else 0.0
                
                # Gross exposure
                portfolio_value = values_df["Portfolio Value"].iloc[-1]
                if portfolio_value > 0:
                    performance_metrics["gross_exposure"] = (total_long + total_short) / portfolio_value
                    performance_metrics["net_exposure"] = (total_long - total_short) / portfolio_value
                else:
                    performance_metrics["gross_exposure"] = 0.0
                    performance_metrics["net_exposure"] = 0.0
